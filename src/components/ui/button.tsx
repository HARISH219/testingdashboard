  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        outline:
          "border border-white/[0.14] bg-transparent hover:bg-white/[0.06] text-foreground",
        ghost: "hover:bg-white/[0.06] text-foreground",
        destructive:
          "bg-destructive text-destructive-foreground hover:brightness-110",
        success: "bg-success text-success-foreground hover:brightness-110",
        discord: "bg-[#5865F2] text-white hover:brightness-110 shadow-glow-sm",
        link: "text-arctic underline-offset-4 hover:underline",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    // Minimal Slot: when asChild is set, merge our classes onto the single
    // child element instead of rendering a <button>. This avoids leaking the
    // `asChild` prop to the DOM and lets <Link>/<a> children be styled as buttons.
